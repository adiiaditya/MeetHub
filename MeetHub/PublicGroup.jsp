...
<%@ page import= "com.User, com.Status, com.AutoComplete, com.MySqlDataStoreUtilities, java.io.*, javax.servlet.*,
...
<%
    ...
    if(session!=null)
    {
      ...
      statusList = MySqlDataStoreUtilities.getUserStatus(email);
			univList = MySqlDataStoreUtilities.getUniversityList();
    }
%>
      ...
      <div class='cover-info'>
       ...
       <img src='images/<%= searchedUser.profilepic %>' />
        ...
        <div class='name'><a href='#'><%= searchedUser.name %></a></div>
                <ul class='cover-nav'>
                  <li><a href='PublicAbout.jsp?searchedUser=<%= searchedUser.email %>'><i class='fa fa-fw fa-user'></i> About</a></li>
                  ...
                  <li><a href='PublicEvents.jsp?searchedUser=<%= searchedUser.email %>'><i class='fa fa-fw fa fa-fw fa-birthday-cake'></i> Events</a></li>
        		      <li class='active'><a href='PublicGroup.jsp?searchedUser=<%= searchedUser.email %>'><i class='fa fa-fw fa fa-fw fa-institution'></i>Groups</a></li>
                </ul>
              </div>
            </div>
                    ...
                    <div class="panel-heading">
                      <div class="media">
                        ...
                        <div class="media-body">
                          ...
                          <a href="UniversityPage.jsp?value1=<%= univList.get(i)%>"><%= univList.get(i)%></a>
						  <%
						  if(MySqlDataStoreUtilities.checkAddedGroup(user.name,univList.get(i))){
							  %><a href="RemoveFromGroup.jsp?value1=<%= univList.get(i)%>"> [Leave -] </a>

						  <%}else{%>
						  <a href="AddToGroup.jsp?value1=<%= univList.get(i)%>"> [Join +] </a>
						  <%}%>
						</div>
                      </div>
                    </div>
